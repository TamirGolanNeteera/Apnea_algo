...
home = '/Neteera/Work/homes/dana.shavit/Research/analysis/cenexel/'
...
if __name__ == "__main__":
    ...
    setups = [ 9643, 9646,9708, 9677, 9709, 9665,9676, 9712, 9604, 9713, 9669, 9710, 9610, 9616, 9617, 9693, 9655]
    #setups = [5699, 5798, 5799, 5728, 6020, 6021, 5755, 6050, 5813, 5812, 6077, 5698, 5810, 6064, 5754, 5931, 6065, 5928, 6076, 6172, 5562, 5598, 5929, 5561, 6053, 5600, 5597, 5563, 6173]
    setups = db.setup_cen_exel_benchmark()
    ...
    for j in setups:
        ...
        sbin = np.max([0, selected_bin-2])
        ebin = np.min([bins.shape[1]-1, selected_bin+3])
        for b in range(sbin, ebin):
            ...
            ax[b-(selected_bin-2)].plot(phase, label="bin "+str(b), color=col, linewidth=0.5)
            ax[b-(selected_bin-2)].legend()
        ax[0].set_title(str(j))

        #plt.show()

        plt.savefig(home+str(j)+"_bins.png", dpi=600)
        plt.close()
