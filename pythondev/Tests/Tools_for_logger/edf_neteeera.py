...
from mne.io.utils import _blk_read_lims, _mult_cal_one
from mne.io.base import BaseRaw
from mne.io.meas_info import _empty_info, _unique_channel_names
...
class RawEDF(BaseRaw):
    ...
    stim_channel : 'auto' | str | list of str | int | list of int
        ...
        mne.io. warning:: 0.18 does not allow for stim channel synthesis from TAL
    ...
    To retrieve correct event values (bits 1-16), one could do:
        ...
        >>> events = mne.find_events(mne.)  # doctest:+SKIP
    ...
    def __init__(self, input_fname, eog=None, misc=None,
        ...
        logger.info('Extracting EDF parameters from {}mne.'.format(input_fname))
        input_fname = os.path.abspath(input_fname)
        info, edf_info, orig_units = _get_info(input_fname,
                                               stim_channel, eog, misc,
                                               exclude, preload)
        logger.info('Creating raw.info structuremne.')
        ...
        if len(edf_info['tal_idx']) > 0:
            ...
            (onset, duration, desc), packets = _read_annotations_edf(tal_data[0])
...
def _read_segment_file(data, idx, fi, start, stop, raw_extras, filenames,
    ...
    with open(filenames, 'rb', buffering=0) as fid:
        ...
        for ai in range(0, len(r_lims), n_per):
            ...
            # Read and reshape to (n_chunks_read, ch0_ch1_ch2_ch3mne.)
...
def _get_info(fname, stim_channel, eog, misc, exclude, preload):
    ...
    logger.info('Setting channel info structuremne.')
    chs = list()
...
def _read_annotations_edf(annotations):
    ...
    return zip(*events), packets if events else (list(), list(), list())
