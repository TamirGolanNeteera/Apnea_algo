...
def df_to_table(slide, df, left=None, top=None, width=None, height=None,
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            cell = shp.table.cell(row, col)
            text = str(val) if col_formatters is None else _do_formatting(val, col_formatters[col])
            if 'HYPERLINK' in text:
                link = text.replace('=HYPERLINK("', '').replace('")', '').split('",')[0]
