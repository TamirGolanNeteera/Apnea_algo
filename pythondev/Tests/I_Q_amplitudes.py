...
if __name__ == "__main__":
    ...
    for i_sess in range(0,len(df['setup'].to_numpy()),2):
        sess = df['setup'].to_numpy()[i_sess]
        setups = df['setup'][df['session'] == sessions[i_sess]].to_numpy()
        versions = df['version'][df['session'] == sessions[i_sess]].to_numpy()
        cfgs = df['config'][df['session'] == sessions[i_sess]].to_numpy()
        ds = df['distance'][df['session'] == sessions[i_sess]].to_numpy()
        if len(setups) != 2:
            print("not ok")
            continue
        ...
        for i_j, j in enumerate(setups):
            ...
            r = []
            abs_data = np.abs(bins)
            rng_var = np.var(abs_data, axis=0)
            for z in range(abs_data.shape[1]):
                r.append(max(200, zero_crossing(abs_data[:,z] - np.mean(abs_data[:,z]))))
            r = np.stack(r)
            # rng_zc = np.array([max(200, zero_crossing(bin_data - np.mean(bin_data)))
            #                    for bin_data in abs_data])
            # eliminate the choice of noisy bin- (noisy= a bin with high zero crossings)
            range_score = rng_var / r / r
            ...
            #ax[i_j].plot(i, label='I', alpha=0.5, linewidth=0.5)
            ...
            #ax[i_j].plot(q, label='Q', alpha=0.5, linewidth=0.5)
            ...
            ax[i_j].set_title(str(setups[i_j]) + ' ' + str(versions[i_j])+' '+cfgs[i_j]+' '+str(ds[i_j])+' median amp. i '+ ' sn '+str(sns[i_j]))
            ax[i_j].grid(True)
            ax[i_j].axis('tight')
            ax[i_j].legend(loc='upper left', fontsize=6)
            mean_bin[i_j].append(np.argmax(np.var(bins, axis=0)))
            mean_bin_zc[i_j].append(np.argmax(range_score))
        #plt.show()
        plt.savefig(home+str(sess)+"_bins.png", dpi=1000)
        plt.close()
    print("130", np.mean(mean_bin[0]), np.mean(mean_bin_zc[0]))
    print("210", np.mean(mean_bin[1]), np.mean(mean_bin_zc[1]))
    df.insert(4, "median_i_amplitude", i_medians, True)
    df.insert(5, "median_q_amplitude", q_medians, True)
    df.to_csv(home+'130plus_pairs.csv')
    print(final_setups)