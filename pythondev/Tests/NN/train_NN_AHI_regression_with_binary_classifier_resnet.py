...
import random
import time as time
from Tests.NN.nn_models import large_resnet_model_late_batchnorm_1D_binary_classifier
...
def my_balanced_augmenting_generator(data_dict, augment_with_chest, batchsize):
    while 1:
        x_out = []
        y_out = []
...
def my_binary_classification_generator(data_dict, batchsize):
    while 1:
        x_out = []
        y_out = []
...
if __name__ == '__main__':
    ...
    for k, v in folders.items():
        ...
        label_files[k] = fnmatch.filter(os.listdir(v), '*_y_binary.*')
    ...
    for k,v in all_data.items():
        y_values.append(all_data[k]['y'])
    y_values = np.hstack(y_values)
    weights = np.histogram(np.hstack(y_values), bins=[0, 1, 4, 8, 16])[0]
    setups_to_run_on = setups if ngpus == 0 else rsetups[gpu_id, :]
    print(setups_to_run_on)
    for k_fold_test_setup in usetups:#setups_to_run_on:
        ...
        for tr in train_sessions:
            ...
            # if 'X_chest' in all_data[tr].keys():
            #     training_data[tr]['X_chest'] = all_data[tr]['X_chest']#[all_data[tr]['valid'] == 1]
            # if 'y_chest' in all_data[tr].keys():
            #     training_data[tr]['y_chest'] = all_data[tr]['y_chest']#[all_data[tr]['valid'] == 1]
        ...
        model = large_resnet_model_late_batchnorm_1D_binary_classifier(input_sig_shape)
        ...
        if not args.reload or failed_load:
            try:
                ...
                model.fit(my_binary_classification_generator(training_data, batch_size),
                      validation_data=my_binary_classification_generator(training_data,  batch_size),
        ...
        for setup_of_subject in radar_setups_all:  # [110606, 110607, 110608, 110609, 110610, 110611]:#radar_setups_all:
            ...
            preds = np.round(model.predict(data_test)).flatten()
            from sklearn.metrics import confusion_matrix
            #print(preds)
            #print(labels_test)
            print(1.0-np.sum(np.abs(preds-labels_test))/len(preds))
            ...
            res_dict[setup_of_subject] = 1.0-np.sum(np.abs(preds-labels_test))/len(preds)
            #print(res_dict)
