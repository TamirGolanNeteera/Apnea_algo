...
import time as time
from Tests.NN.nn_models import create_vit_BP_regressor_8d
...
def my_weighted_generator(data_dict, batchsize):
    while 1:
        ...
        for k in range(batchsize):
            ...
            x_out.append(v['X'][cur_select])
            y_out.append(v['y'][cur_select])#[0])the [0] was when both sbp and dbp
...
if __name__ == '__main__':
    ...
    for i, fn in enumerate(label_files):
        ...
        all_data[sess]['y'] = all_data[sess]['y'][:,0]
        all_data[sess]['y'] = [int(s) for s in all_data[sess]['y']]
    subjects_to_run_on = usubjects if ngpus == 0 else rsubjects[args.gpu-1, :]
    ...
    for k_fold_test_subject in subjects_to_run_on:
        ...
        model = create_vit_BP_regressor_8d(input_shape)
        model.summary()
