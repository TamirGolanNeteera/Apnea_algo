...
from Tests.NN.create_apnea_count_AHI_data import delays, MB_HQ, count_apneas_in_chunk, getSetupRespirationCloudDBDebug, getSetupRespirationCloudDB, getApneaSegments
...
import stumpy
def get_noise_mask(signal, time_chunk_in_seconds, frame_rate):
    time_chunk_in_samples = time_chunk_in_seconds * frame_rate
    noise_mask = np.zeros(len(signal))
    pattern_length_in_seconds = 12 * 3 #very very roughly, 3 breaths

    for i in range(time_chunk_in_samples, len(signal), time_chunk_in_samples):

        m = frame_rate * pattern_length_in_seconds  # size of repeating pattern to look for

        resp = signal[i - time_chunk_in_samples:i]
        nm = noise_mask[i - time_chunk_in_samples:i]
        # fig, axs = plt.subplots(3, sharex=False, gridspec_kw={'hspace': 0})
        mp = stumpy.stump(resp, m, normalize=False)

        motif_index = np.argsort(mp[:, 0])[0]  # lowest subsequence distance to most similar subsequence
        #motif_onset = mp[motif_index, 1]
        #nearest_neighbor_idx = mp[motif_index, 1]
        #motif = resp[motif_index:motif_index + m]

        mp0 = mp[:, 0]
        noise_mask[i - time_chunk_in_samples:i - time_chunk_in_samples + len(mp0)] = mp0
    return noise_mask
...
if __name__ == '__main__':
    ...
    setups = MB_HQ#[min(db.setups_by_session(session)) for session in sessions][3:]

    if args.show:
        setups = [110580, 110581,110582 ]
    print(setups)
    for i_sess, sess in enumerate(setups):
        ...
        if args.show:
            ...
            nm = get_noise_mask(respiration, 15*60, fs_new)


            fig, ax = plt.subplots(3, sharex=True)

            binss = np.repeat(bins, 10)
            ax[0].plot(binss)
        ...
        try:
            ...
            respiration, fs_new, bins = getSetupRespirationCloudDB(sess, down, lp, hp)
            ...
            for ii in range(len(empty_ref_mb)):
                if bins[ii] == 1:
                    empty_ref_mb[ii] = 1
