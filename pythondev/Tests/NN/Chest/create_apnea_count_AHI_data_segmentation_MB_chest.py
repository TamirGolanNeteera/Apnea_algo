...
def create_AHI_segmentation_training_data_MB_chest(respiration, apnea_ref, time_chunk, step, scale, fs):
    X = []
    y = []
    valid = []
    print("inside ",np.unique(apnea_ref))
    for i in range(time_chunk, len(respiration), step):
        ...
        if len(labels[labels < 0]):
            v = v * 0
        #print(int((i - time_chunk) / fs), int(i / fs), len(labels), np.unique(labels))
...
if __name__ == '__main__':
    ...
    for i_sess, sess in enumerate(setups):
        ...
        try:
            ...
            abs = np.load(abs_path, allow_pickle=True).asfreq('100ms', method='pad')
        except:
            print(sess, "not ok")
            continue
        ...
        apnea_ref = apnea_ref.resample('1S').mean()
        X, y, valid = create_AHI_segmentation_training_data_MB_chest(respiration=abs, apnea_ref=apnea_ref,
