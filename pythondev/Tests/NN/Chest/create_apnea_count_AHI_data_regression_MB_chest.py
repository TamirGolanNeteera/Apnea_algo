...
from Tests.NN.create_apnea_count_AHI_data import count_apneas_in_chunk, apnea_class
...
def create_AHI_regression_training_data_MB_chest(respiration, apnea_ref, time_chunk, step, scale, fs):
    ...
    return X,y, valid
#
def create_AHI_regression_training_data_MB_chest_fs(respiration, apnea_ref, time_chunk, step, scale, fs):
    X = []
    y = []
    valid = []
    apnea_segments = []
    ...
    for i in range(time_chunk, len(respiration), step):
        ...
        num_apneas = count_apneas_in_chunk(start_t=int((i - time_chunk)/fs), end_t=int(i/fs), apnea_segments=apnea_segments)
    ...
    for i_sess, sess in enumerate(setups):
        ...
        try:
            ...
            for file in os.listdir(setup_ref_path):
                if 'abdominal' in file:
                    abs_path = os.path.join(setup_ref_path, file)
                    break

            ch = np.load(abs_path, allow_pickle=True).asfreq('100ms', method='pad')
        except:
            print(sess, "not ok 2")
            continue
        reference = reference.asfreq('100ms', method='pad')
        fs_ref = get_fs(reference)
        # gaps, start = get_gaps(reference.chest, reference.apnea, fs_ref)
        # print(gaps)
        apnea_with_gaps = reference.apnea
        # for base, gap_len in gaps.items():
        #     print(gap_len)
        #     apnea_with_gaps = pd.concat([apnea_with_gaps.iloc[:base + start],
        #                                  pd.DataFrame(index=range(gap_len)),
        #                                  apnea_with_gaps.iloc[base + start:]])

        apnea_ref = (apnea_with_gaps.notna() & (apnea_with_gaps != 'normal')) * 1
        #plt.plot(range(len(ch)), ch)#2*ch/max(ch))
        ...
        X, y, valid = create_AHI_regression_training_data_MB_chest(respiration=ch, apnea_ref=apnea_ref,
