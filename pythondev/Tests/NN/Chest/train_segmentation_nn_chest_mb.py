...
def my_balanced_generator0(data_dict, batchsize):
    while 1:
        x_out = []
        y_out = []
...
if __name__ == '__main__':
    ...
    #     train_path = train_path + '_unscaled'
    # if args.filter_wake_from_train:
    #     load_path = os.path.join(load_path, 'filter_wake_from_train')
    #     train_path = train_path + '_filter_wake'
    # else:
    #     load_path = os.path.join(load_path, 'unscaled')
    #     train_path = train_path + '_no_filter_wake'
    ...
    for _ in [1]:
        ...
        refs_path = load_path.replace("signals", "references")
        label_files = fnmatch.filter(os.listdir(refs_path), '*_y.*')
        valid_files = fnmatch.filter(os.listdir(refs_path), '*_valid*.*')
        print(data_files)
        print(label_files)
        print(valid_files)
        ...
        for i, fn in enumerate(label_files):
            ...
            all_data[sess]['y'] = np.load(os.path.join(refs_path, fn), allow_pickle=True)
            print(sess, "y", np.unique(all_data[sess]['y']))
        for i, fn in enumerate(valid_files):
            sess = int(fn[0:6])

            if sess not in all_data.keys():
                continue
            all_data[sess]['valid'] = np.load(os.path.join(refs_path, fn), allow_pickle=True)


        json_fn = '/model/' + '_model.json'
        weights_fn = '/model/' + '_model.hdf5'
        if True:
            ...
            checkpointer = ModelCheckpoint(filepath=train_path + '/checkpoints/'+'_model.hdf5', verbose=1, save_best_only=True)
            log_dir = train_path +  "/" +"_logs/fit/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
            ...
            if not args.reload or failed_load:
                ...
                #continue
        # test the NNf
        for setup_of_subject in all_setups:
            ...
            fn = 'all2_' + str(setup_of_subject) + '_unet_preds_no_wake_valid.png'
    ...
    fn = 'all_' + str(setup_of_subject) + '_unet_preds_vs_gt2_valid.png'
    ...
    plt.figure()
    plt.scatter(hourly_gt, hourly_preds, s=3)
