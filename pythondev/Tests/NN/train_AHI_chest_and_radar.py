...
from Tests.NN.create_apnea_count_AHI_data_regression_MB_phase_filter_empty import create_AHI_regression_training_data_MB_phase, apnea_class, get_empty_seconds_mb
...
nw_setups = [8998, 9188, 9003, 8674, 8995, 8580, 9093, 8999, 9187, 9002, 6284, 9100, 8719, 8757, 8920, 6651, 9053, 8735,
                  8710, 8584, 8994, 8579, 9094,  9101, 8724, 8756, 8919, 6641, 9050]
        ...
        for k in range(batchsize):
            ...
            if sessions[i] in mb_setups:
                if np.random.rand() > 0.5:
                    ...
                    cur_select = np.random.choice(len(v['y_ch']), 1)[0]
                    y = v['y_ch'][cur_select]
                    data_place = v['X_ch']
                    label_place = v['y_ch']
            else:
            ...
            if y > 5 and np.random.rand() > 0.5:
                if np.random.rand() > 0.5:
                    x_o = np.zeros_like(data_place[cur_select])
                    half = int(len(x_o) / 2)

                    x_o[:half] = data_place[cur_select][half:]
                    x_o[half:] = data_place[cur_select][:half]
                else:
                    x_o = np.zeros_like(data_place[cur_select])
                ...
                if y == 0 and np.random.rand() > 0.5:
                    ...
                    x_out.append(data_place[cur_select])
            y_out.append(label_place[cur_select])
            #print(i, cur_select, len(x_out), len(y_out))
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--nw_first', action='store_true', help='scale test vectors to m=0, s=1')
...
if __name__ == '__main__':
    ...
    load_path_ch = args.load_path_ch
    ...
    if args.scale:
        ...
        load_path_ch = os.path.join(load_path_ch, 'scaled')
        ...
        load_path_ch = os.path.join(load_path_ch, 'unscaled')
    ...
    res_mae = {}
    mb_rev = mb_setups
    all_setups = nw_setups[::-1]  + mb_rev if args.nw_first else  mb_rev + nw_setups
    print(len(all_setups), "setups")

    if True:
        #print("::::::::::::::::::::", k_fold_test_setup, "::::::::::::::::::::")
        json_fn = '/model/' + str(0) + '_model.json'
        weights_fn = '/model/' + str(0) + '_model.hdf5'
        all_setups = nw_setups[::-1] + mb_rev if args.nw_first else mb_rev + nw_setups
        print(len(all_setups), "setups")
        for k_fold_test_setup in all_setups:
            ...
            data_files_ch = fnmatch.filter(os.listdir(load_path_ch), '*_X.*')
            label_files_ch = fnmatch.filter(os.listdir(load_path_ch), '*_y.*')
            valid_files_ch = fnmatch.filter(os.listdir(load_path_ch), '*_valid.*')
            ...
            # load chest data
            for i, fn in enumerate(data_files_ch):
                sess = int(fn[0:fn.find('_')])

                if int(sess) not in all_data.keys():
                    all_data[sess] = {}
                all_data[sess]['X_ch'] = np.load(os.path.join(load_path_ch, fn), allow_pickle=True)
                if np.isnan(all_data[sess]['X_ch']).any():
                    print(sess, "X contains nan")

            for i, fn in enumerate(label_files_ch):
                sess = int(fn[0:fn.find('_')])

                if sess not in all_data.keys():
                    continue
                all_data[sess]['y_ch'] = np.load(os.path.join(load_path_ch, fn), allow_pickle=True)
                #(sess, all_data[sess]['y'].shape)
                if (all_data[sess]['y_ch'] == None).any() or np.isnan(all_data[sess]['y_ch']).any():
                    print(sess, "y contains nan")

            for i, fn in enumerate(valid_files_ch):
                sess = int(fn[0:fn.find('_')])

                if sess not in all_data.keys():
                    continue
                all_data[sess]['valid_ch'] = np.load(os.path.join(load_path_ch, fn), allow_pickle=True)
                if np.isnan(all_data[sess]['valid_ch']).any():
                    print(sess, "valid contains nan")
            print("::::::::::::::::::::", k_fold_test_setup, "::::::::::::::::::::")
            ...
            if True:
                ...
                val_sessions = train_sessions  # and s not in benchmark_setups]
                ...
                for tr in train_sessions:
                    ...
                    if tr not in nw_setups:
                        training_data[tr]['X_ch'] = all_data[tr]['X_ch']#[all_data[tr]['valid_ch'] == 1]
                        training_data[tr]['y_ch'] = all_data[tr]['y_ch']#[all_data[tr]['valid_ch'] == 1]
            ...
            for setup_of_subject in radar_setups_all:
                ...
                mae = np.round(np.sum(np.abs(preds - labels_test)) / len(preds), )
                mae_valid = np.round(np.sum(np.abs(preds - labels_test)[valid == 1]) / len(valid[valid == 1]), 2)
                ...
                for i, seg in enumerate(data_test):
                    if err[i] < 1:
                        fn = 'good_' + str(setup_of_subject) + '_' + str(i) + '_' + str(
                            err_signed[i]) + '_valid_' + str(valid[i]) + '.npy'
                        np.save(os.path.join(args.save_path, fn), seg)
                    elif err[i] > 15:
                        fn = 'bad_' + str(setup_of_subject) + '_' + str(i) + '_' + str(err_signed[i]) + '_valid_' + str(
                            valid[i]) + '.npy'
                        np.save(os.path.join(args.save_path, fn), seg)
                ...
                res_mae[setup_of_subject] = [mae, mae_valid]
