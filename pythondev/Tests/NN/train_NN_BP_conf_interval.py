...
import time as time
from Tests.NN.nn_models import small_vgg_model, large_vgg_model_late_batchnorm, large_vgg_model_late_batchnorm_test_time_dropout
...
def create_dropout_predict_function(model, dropout):
    ...
    predict_with_dropout = K.function(model_dropout.inputs , model_dropout.outputs)#+ [K.learning_phase()]
...
if __name__ == '__main__':
    ...
    for k_fold_test_setup in setups:
        ...
        model = large_vgg_model_late_batchnorm_test_time_dropout(input_shape)
        ...
        for setup_of_subject in all_subject_setups:
            ...
            preds_stack = np.zeros((25, all_data[setup_of_subject]['y'].shape[0], 1))
            for pi in range(25):
                preds_stack[pi,:] = model.predict(data_test)
            preds = model.predict(data_test)
            preds = preds.flatten()
            print(np.mean(preds_stack, axis=0))
            ...
            np.save(os.path.join(args.save_path, str(setup_of_subject) + '_pred_mean.npy'), np.mean(preds_stack, axis=0), allow_pickle=True)
            np.save(os.path.join(args.save_path, str(setup_of_subject) + '_pred_std.npy'), np.std(preds_stack, axis=0), allow_pickle=True)
            ...
            plt.plot(np.mean(preds_stack, axis=0), label='pred mean')
