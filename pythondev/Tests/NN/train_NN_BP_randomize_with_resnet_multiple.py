...
import time as time
from Tests.NN.nn_models import small_vgg_model, large_resnet_model_late_batchnorm_multiple_outputs,large_resnet_model_late_batchnorm, small_resnet_model_late_batchnorm
...
def my_weighted_generator(data_dict, batchsize):
    while 1:
        ...
        for k in range(batchsize):
            ...
            x_out.append(v['X'][cur_select])
            y_out.append(np.multiply(np.ones(8), v['y'][cur_select][0]))
        print(y_out.shape, y_out)
        x_out = np.asarray(x_out)
...
if __name__ == '__main__':
    ...
    for k_fold_test_setup in setups:
        ...
        #model_json_file = '/Neteera/Work/homes/dana.shavit/Research/BP2023/trained_nns/030423_resnet_mse/model/P2_model.json'# large_resnet_model_late_batchnorm(input_shape)
        #json_file = open(model_json_file, 'r')
        #model_json = json_file.read()
        #json_file.close()
        #model = keras.models.model_from_json(model_json)
        model = large_resnet_model_late_batchnorm_multiple_outputs(input_shape)
        model.summary()
