...
def my_crazy_new_generator(data_dict, batchsize):
    while 1:
        x_out = []
        y_out = []
        ...
        for k in range(int(batchsize/2)):
            i = np.random.choice(len(sessions), 1)[0]
            v = data_dict[sessions[i]]
            cur_select = np.random.choice(len(v['y']), 1)[0]
            y = v['y'][cur_select]
        ...
        for k in range(int(batchsize/2)):
            i = np.random.choice(len(sessions), 1)[0]
            v = data_dict[sessions[i]]
            cur_select1 = np.random.choice(len(v['y']), 1)[0]
            cur_select2 = np.random.choice(len(v['y']), 1)[0]
            cur_select3 = np.random.choice(len(v['y']), 1)[0]
            y3_1 = v['y3'][cur_select1]
            y3_2 = v['y3'][cur_select2]
            y3_3 = v['y3'][cur_select3]
            x_o = np.zeros_like(v['X'][cur_select1])
            max_seg1 = np.argmax(y3_1)
            third = 3000
            x_o[:third] = v['X'][cur_select1][max_seg1*third:(max_seg1+1)*third]
            max_seg2 = np.argmax(y3_2)
            x_o[third:2*third] = v['X'][cur_select2][max_seg2*third:(max_seg2+1)*third]
            max_seg3 = np.argmax(y3_3)
            x_o[2*third:3*third] = v['X'][cur_select3][max_seg3*third:(max_seg3+1)*third]
            x_out.append(x_o)
            y_out.append(y3_1[max_seg1] + y3_2[max_seg2] + y3_3[max_seg3])
...
if __name__ == '__main__':
 ...
 #109901, 109903, 109906, 109910, 109918, 109928, 109937, 109958, 109966, 110033, 110044, 110045, 110071, 110072, 110190, 110191]
    ...
    res_mae = {}
    mb_rev = mb_setups#[::-1]
    all_setups = nw_setups  + mb_rev if args.nw_first else  mb_rev + nw_setups
    ...
    for k_fold_test_setup in all_setups:
        ...
        label3_files_nw = fnmatch.filter(os.listdir(refs_path_nw), '*_y3.*')
        ...
        label3_files_mb = fnmatch.filter(os.listdir(refs_path_mb), '*_y3.*')
        valid_files_mb = fnmatch.filter(os.listdir(refs_path_mb), '*_valid.*')


        #print(data_files_mb)
        print("label3_files_mb",label3_files_mb)
        print("label3_files_nw",label3_files_nw)
        ...
        for i, fn in enumerate(label3_files_nw):
            sess = int(fn[0:fn.find('_')])
            if sess in [k_fold_test_setup] or sess == 8705 :
                continue
            if sess not in all_data.keys():
                continue
            all_data[sess]['y3'] = np.load(os.path.join(refs_path_nw, fn), allow_pickle=True)
            if np.isnan(all_data[sess]['y3']).any():
                print(sess, "y3 contains nan")
        ...
        for i, fn in enumerate(label3_files_mb):
            sess = int(fn[0:fn.find('_')])

            if sess not in all_data.keys():
                continue
            all_data[sess]['y3'] = np.load(os.path.join(refs_path_mb, fn), allow_pickle=True)
            #(sess, all_data[sess]['y'].shape)
            if (all_data[sess]['y3'] == None).any() or np.isnan(all_data[sess]['y3']).any():
                print(sess, "y3 contains nan")
        ...
        if True:
           ...
           # print("val", val_sessions)
            ...
            for tr in train_sessions:
                ...
                training_data[tr]['y3'] = all_data[tr]['y3'][all_data[tr]['valid'] == 1]
            ...
            if not args.reload or failed_load:
                ...
                if args.augment:
                    model.fit(my_crazy_new_generator(training_data, batch_size),

                              # validation_data=my_generator(data_train, labels_train, batch_size),
                              validation_data=my_crazy_new_generator(training_data, batch_size),
