...
import time as time
from Tests.NN.nn_models import *
...
def my_balanced_loader(setups, data_path, batchsize):
    while 1:
        x_out = []
        y_out = []
        ...
        for k in range(batchsize):
            ...
            X = np.load(os.path.join(data_path, str(setup)+'_X.npy'), allow_pickle=True)
            y = np.load(os.path.join(data_path, str(setup)+'_y.npy'), allow_pickle=True)
            #v = data_dict[sessions[i]]
            cur_select = np.random.choice(len(y), 1)[0]
            #print(X[cur_select].shape, y[cur_select].shape)
            x_out.append(X[cur_select].T)
            y_out.append(y[cur_select][0].T)
...
if __name__ == '__main__':
    ...
    for k_fold_test_setup in setups:
        ...
        print("::::::::::::::::::::", k_fold_test_setup, db.setup_subject(k_fold_test_setup), "::::::::::::::::::::")
        ...
        model = large_vgg_model_late_batchnorm_more_pooling(input_shape)
        ...
        if not args.reload or failed_load:
            model.fit(my_balanced_loader(setups, args.load_path, batch_size),

                  #validation_data=my_generator(data_train, labels_train, batch_size),
                  #validation_data=my_balanced_loader(setups, args.load_path, batch_size),
