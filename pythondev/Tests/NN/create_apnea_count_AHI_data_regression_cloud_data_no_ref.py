...
def create_AHI_regression_training_data_no_ref(respiration, empty_seconds, time_chunk, step, scale, fs):
    print("in")
    X = []
    valid = []
    ...
    if len(X):
        ...
        valid = np.stack(valid)
        print(np.count_nonzero(valid))
        print(len(X), np.sum(valid) / len(valid))
        return X, valid
...
if __name__ == '__main__':
    ...
    for i_sess, sess in enumerate(setups):
        ...
        try:
            ...
            X, valid,  = create_AHI_regression_training_data_no_ref(respiration=respiration,
            ...
            continue
        print(X.shape)
        print("successfully created AHI data")
