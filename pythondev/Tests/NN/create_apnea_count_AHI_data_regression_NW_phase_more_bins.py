...
from create_apnea_count_AHI_data import MB_HQ, create_AHI_training_data, create_AHI_training_data_no_wake_no_empty, count_apneas_in_chunk, getApneaSegments, getWakeSegments, getSetupRespiration,getSetupRespiration_top3bins
...
if __name__ == '__main__':
    ...
    for i_sess, sess in enumerate(setups):
        ...
        try:
            ...
            respiration, fs_new = getSetupRespiration_top3bins(sess, down, lp, hp)
            min_setup_length = 15000
            if respiration.shape[1] / fs_new < min_setup_length:
                continue
            ...
            for b in range(respiration.shape[0]):
                ax[b].plot(respiration[b,:])
                X, y, valid = create_AHI_regression_training_data_NW_phase(respiration=respiration[b,:],
