...
from Tests.NN.nn_models import smaller_vgg_model_1d
from datetime import datetime

now = datetime.now()
strtime = now.strftime('%d_%m_%y_%H_%M_%S')
...
if __name__ == '__main__':
    ...
    all_subj =np.unique([db.setup_subject(s) for s in setups])


    random.shuffle(all_subj)
    cuttoff = int(0.8*len(all_subj))
    train_subj = all_subj[:cuttoff]
    test_subj = all_subj[cuttoff:]

    train_sessions = [s for s in setups if db.setup_subject(s) in train_subj]
    test_sessions = [s for s in setups if db.setup_subject(s) not in train_subj]

    print("len(train_sessions)",len(train_sessions))
    print("len(test_sessions)",len(test_sessions))
    file_dict = {}
    all_data = {}
    mean_dict = {}
    ...
    model = smaller_vgg_model_1d(input_shape)
    ...
    if not args.reload or failed_load:
        try:
            ...
            np.save(os.path.join(args.save_path, "train_setups.npy"), train_sessions, allow_pickle=True)
            np.save(os.path.join(args.save_path, "test_setups.npy"), test_sessions, allow_pickle=True)
            print("saved session lists to disk")
        except:
...
# test the NN
    ...
    if args.test:
        ...
        train_sessions = np.load(os.path.join(args.save_path, "train_setups.npy"), allow_pickle=True)
        test_sessions = np.load(os.path.join(args.save_path, "test_setups.npy"), allow_pickle=True)
        ...
        for i,setup_of_ds in enumerate(test_sessions):
            ...
            # plt.plot(gt_vec, linewidth=0.5, c='green', label='gt')
            # plt.plot(pred_vec, linewidth=0.5, c='orange', label='pred')
            # plt.legend()

            np.save(os.path.join(args.save_path,res_fn_y_true), y_true)
            np.save(os.path.join(args.save_path,res_fn_y_pred), y_pred)
        print(np.mean(front_err),front_err)
        print(np.mean(back_err),back_err)
        plt.savefig(os.path.join(args.save_path, ds + "_out.png"))
        plt.close()
