...
def my_weighted_generator_for_filtered_data(data_dict, batchsize):
    ...
    while 1:
        ...
        for k in range(batchsize):
            ...
            label_data = np.load(os.path.join(data_dict['load_path'], filenames[1]))

            if len(X) != 2500:
                continue
            x_out.append(X)
            y_out.append(float(label_data[0]))
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--plot', action='store_true', help='test NN')
...
if __name__ == '__main__':
    ...
    for ds in datasets:
        ...
        if not args.reload or failed_load:
            try:
                model.compile(loss=args.loss, optimizer=keras.optimizers.Adam())
                model.fit(my_weighted_generator_for_filtered_data(data_dict, batch_size),
                      validation_data=my_weighted_generator_for_filtered_data(data_dict, 64),
    ...
    # test the NN
        ...
        if args.test:
            ...
            for i,setup_of_ds in enumerate(test_sessions):
                ...
                for f in data_files:
                    ...
                    try:
                        ...
                        continue
                    all_X.append(X)
                    gt = np.load(os.path.join(args.load_path,y_fn), allow_pickle=True)
                    y_true.append(float(gt[0]))#[y[i], st, q]
                    stat.append(gt[1])
                    hq.append(int(gt[2]))
                ...
                try:
                    ...
                    y_stat = y[stat=='running']
                    gt_stat = y_true[stat=='running']
                    y_hq = y[hq == 1]
                    gt_hq = y_true[hq == 1]
                except:
                    print("no X")
                ...
                stat_vec = np.stack(stat)
                hq_vec = np.stack(hq)
                stat_vec[stat_vec=='normal']='full'
                stat_vec[stat_vec=='running']='full'
                err = np.abs(np.nanmean(gt_vec)-np.nanmean(pred_vec))
                err_stat = np.abs(np.nanmean(gt_vec[stat_vec=='full'])-np.nanmean(pred_vec[stat_vec=='full']))
                err_hq = np.abs(np.nanmean(gt_vec[hq_vec==1])-np.nanmean(pred_vec[hq_vec==1]))
                ...
                errs.append(err)
                errs_stat.append(err_stat)
                errs_hq.append(err_hq)
                s =  np.isnan(err_stat)
                ...
                if np.mean(gt_vec) < 35:
                    ...
                    continue
                if args.plot:
                    # ax[0].scatter(np.nanmean(gt_vec), np.mean(pred_vec), s=5, c=col[i % len(col)])
                    # ax[1].scatter(np.mean(gt_vec[hq_vec == 1]), np.mean(pred_vec[hq_vec == 1]), s=5, c=col[i % len(col)])
                    # ax[2].scatter(np.mean(gt_vec[stat_vec == 'full']), np.mean(pred_vec[stat_vec == 'full']), s=5, c=col[i % len(col)])
                    ax[0].scatter(gt_vec, pred_vec, s=2, c=col[i % len(col)])
                    ax[1].scatter(gt_vec[hq_vec == 1], pred_vec[hq_vec == 1], s=2, c=col[i % len(col)])
                    ax[2].scatter(gt_vec[stat_vec == 'full'], pred_vec[stat_vec == 'full'], s=2, c=col[i % len(col)])
            ...
            err_mean = np.nanmean(errs)
            stat_err_mean = np.nanmean(errs_stat)
            hqerr_mean = np.nanmean(errs_hq)
            if args.plot:
                ...
                ax[0].set_title(ds + ' err(all) : ' + str(err_mean))
                ax[1].set_title(ds + ' err(hq==1): ' + str(stat_err_mean))
                ax[2].set_title(ds + ' err(stat==full): ' + str(hqerr_mean))
                plt.savefig(os.path.join(args.save_path, "out_" + ds + "_by_point.png"))
                print("saved image")
                plt.close()
