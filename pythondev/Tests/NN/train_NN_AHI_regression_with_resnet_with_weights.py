...
def my_balanced_augmenting_generator(data_dict, augment_with_chest, batchsize):
    while 1:
        ...
        for k in range(batchsize):
            ...
            # if augment_with_chest and 'X_chest' in v.keys():
...
if __name__ == '__main__':
    ...
    for k, v in folders.items():
        for j, d in enumerate([data_files, label_files, valid_files]):
    ...
    for k_fold_test_setup in setups_to_run_on:
        ...
        for k, v in training_data.items():
            y_values.append(training_data[k]['y'])
        y_values = np.hstack(y_values)
        bins = [0, 1, 4, 8, 16, 50]
        weights = np.histogram(np.hstack(y_values), bins=bins)[0]
        #weights = [len(v)/sum_setups for v in list(setups_by_bp.values())]
        print(weights)
        inv_weights = np.array([1.0/(w+1e-9) for w in weights])

        weights  = inv_weights/np.sum(inv_weights)
        data_by_label = {}
        for w in bins:
            data_by_label[w] = {'X': [], 'y': [], 'valid': []}
        ...
        for k, v in training_data.items():
            ...
            for i, y in enumerate(v['y']):
                ...
                for w in range(len(bins)):
                    ...
                    if y >= bins[w - 1] and y < bins[w]:
                        data_by_label[bins[w - 1]]['X'].append(v['X'][i])
                        data_by_label[bins[w - 1]]['y'].append(v['y'][i])
                        #data_by_label[bins[w - 1]]['valid'].append(v['valid'][i])

                        break

        data_by_label.pop(50)
        epochs = 500
        batch_size = 128
        ...
        failed_load = False
        data_dict = {'weights':weights, 'category_list': bins, 'data':data_by_label}
        if args.reload:
            try:
        ...
        if not args.reload or failed_load:
            try:
                ...
                model.fit(my_weighted_generator(data_dict, batch_size),
                      validation_data=my_weighted_generator(data_dict, batch_size),
