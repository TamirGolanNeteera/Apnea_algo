...
from keras.layers import Conv2D, MaxPool2D, Conv1D, GRU, LSTM,  Bidirectional, MaxPool1D, MaxPool1D, BatchNormalization, Dropout
...
def my_balanced_generator(data_dict, batchsize):
    while 1:
        ...
        for k in range(batchsize):
            ...
            pselect = min(1.0, 0.25 + v['y'][cur_select]/20.0)
            if np.random.rand() > pselect:
                continue
...
def my_classification_generator(data_dict, batchsize):
    while 1:
        x_out = []
        y_out = []
...
if __name__ == '__main__':
    ...
    nw_setups = [8999, 9187, 9002, 8674, 8995, 8580, 9093, 8998, 9188, 9003, 6284, 9100, 8719, 8757, 8920, 6651, 9053, 8735,
 ...
 #109901, 109903, 109906, 109910, 109918, 109928, 109937, 109958, 109966, 110033, 110044, 110045, 110071, 110072, 110190, 110191]
    ...
    accuracies = []
    res = {}
    all_setups = nw_setups  + mb_setups if args.nw_first else  mb_setups + nw_setups
    ...
    for k_fold_test_setup in all_setups:
        ...
        if True:
           ...
           # print("val", val_sessions)
            ...
            model.compile(loss=args.loss, optimizer=keras.optimizers.Adam(),metrics=['accuracy'])
            ...
            if not args.reload or failed_load:
                ...
                model.fit(my_classification_generator(training_data, batch_size),

                          #validation_data=my_generator(data_train, labels_train, batch_size),
                          validation_data=my_classification_generator(training_data, batch_size),
