    ...
    while 1:
        ...
        for k in range(batchsize):
            ...
            if y > 5 and np.random.rand() > 0.5:
                x_o = np.zeros_like(vx[cur_select])

                half = int(len(x_o)/2)

                x_o[:half] = vx[cur_select][half:]
                x_o[half:] = vx[cur_select][:half]
                x_out.append(x_o)
            else:
    ...
    for k_fold_test_setup in all_setups:
        ...
        for i, fn in enumerate(data_files_nw):
            ...
            if int(sess) not in all_data.keys():
                all_data[sess] = {}
                all_data[sess]['X'] = []
            all_data[sess]['X'].append(np.load(os.path.join(load_path_nw, fn), allow_pickle=True))
            if np.isnan(all_data[sess]['X']).any():
                print(sess, "X contains nan")
        #all_data[sess]['X'] = np.stack(all_data[sess]['X'])
        ...
        for i, fn in enumerate(valid_files_nw):
            ...
            if sess not in all_data.keys():
                continue
            if 'valid' in all_data[sess].keys():
                continue
            ...
            if setup_of_subject in nw_setups:
                ...
                if setup_of_subject > 110000:
                    ...
                    data_test, labels_test, valid = create_AHI_regression_training_data_MB_phase(
                        respiration=respiration, apnea_ref=None, empty_seconds=None,
                        time_chunk=time_chunk, step=step, scale=args.scale, fs=fs_new)

                    preds = model.predict(data_test)
                    #print(setup_of_subject, np.mean(preds) * 4)
                    #continue
