    ...
    while 1:
        ...
        for k in range(batchsize):
            ...
            if y > 5 and np.random.rand() > 0.5:
                ...
                if y == 0 and np.random.rand() > 0.25:
                    continue
                else:
                    x_out.append(v['X'][cur_select])
            y_out.append(v['y'][cur_select])
 ...
 #109901, 109903, 109906, 109910, 109918, 109928, 109937, 109958, 109966, 110033, 110044, 110045, 110071, 110072, 110190, 110191]
    ...
    if True:
        ...
        if True:
            ...
            train_sessions = [s for s in all_data.keys() if s in mb_setups]# and s in all_setups]
            val_sessions = [s for s in train_sessions]# and s not in benchmark_setups]
            excluded_sessions = [s for s in all_data.keys() if s  in nw_setups]
           ...
           # print("val", val_sessions)
            ...
            checkpointer = ModelCheckpoint(
                filepath=train_path + '/checkpoints/' + str(0) + '_model.hdf5', verbose=1,
                save_best_only=True)
            log_dir = train_path + "/" + str(0) + "_logs/fit/" + datetime.datetime.now().strftime(
                "%Y%m%d-%H%M%S")
            tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1)
            model_json = model.to_json()
            with open(train_path + '/checkpoints/' + str(0) + 'model.json', "w") as json_file:
            ...
            if args.reload:
                try:
                    ...
                    model.compile(loss=args.loss, optimizer=keras.optimizers.Adam())
                    print(0, "successfully loaded model from storage")
                except:
