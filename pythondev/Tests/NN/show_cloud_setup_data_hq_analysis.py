...
from local_DB_API.vsms_db_api import *
...
base_path = '/Neteera/Work/homes/dana.shavit/Research/analysis/3.6.xx/carmel/'
...
if __name__ == '__main__':
    ...
    for i_sess, sess in enumerate(setups):
        ...
        bin_change = np.abs(np.diff(bins,prepend=0))
        ...
        if plot_empty_data:
            ...
            for t in range(time_seg, len(bin_change), time_seg):
                bins_change_rate.append(np.sum(bin_change[t - time_seg:t][bin_change[t - time_seg:t] != 0]) / time_seg)
                hq_rate.append(np.sum(hr_hq[t - time_seg:t][hr_hq[t - time_seg:t] != 0]) / time_seg)
                amp_means.append(np.mean(amp_sec[t - time_seg:t]))
    ...
    bins_change_rate =np.stack(bins_change_rate)
    hq_rate = np.stack(hq_rate)
    amp_means = np.stack(amp_means)
    plt.figure()
    plt.scatter(bins_change_rate, hq_rate, s=5)
    plt.figure()
    plt.scatter(amp_means, bins_change_rate, s=5)
    plt.show()