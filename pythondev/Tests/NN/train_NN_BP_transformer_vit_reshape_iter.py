...
if __name__ == '__main__':
    ...
    for k_fold_test_subject in subjects_to_run_on:
        ...
        for epo in range(10):
            ...
            input_shape = (all_data[tr]['X'][0].shape[1], 1,all_data[tr]['X'][0].shape[0])

            model = create_vit_classifier_1d(input_shape)
            #model.summary()
            # model_from_good_json = '/Neteera/Work/homes/dana.shavit/Research/BP2023/trained_nns/200423_transformer_tokenizr_1d_32_mae/model/orit_model.json'
            # model_json_from = model_from_good_json.read()
            # model_from_good_json.close()
            # model_from = keras.models.model_from_json(model_json_from)
            # model_from.compile(loss=args.loss, optimizer=keras.optimizers.Adam())
            #
            # model_from.summary()
