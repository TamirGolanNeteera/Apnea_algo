...
if __name__ == '__main__':
    ...
    if args.scale:
        ...
        load_path_nw = os.path.join(args.load_path_nw, 'scaled/filter_wake_from_train')
    #     train_path = train_path + '_scaled'
    else:
        load_path_mb = os.path.join(args.load_path_mb, 'unscaled')
        load_path_nw = os.path.join(args.load_path_nw, 'unscaled/filter_wake_from_train')
    ...
    mb_setups = [109816, 109870, 109872, 109877, 109884, 109886, 109887, 109889, 109892, 109897,
    ...
    for k_fold_test_setup in all_setups:
        ...
        if True:
            ...
            checkpointer = ModelCheckpoint(
                filepath=train_path + '/checkpoints/' + str(k_fold_test_setup) + 'model.hdf5', verbose=1,
                save_best_only=True)
        ...
        for setup_of_subject in subject_setups:
            ...
            # if setup_of_subject in mb_setups:
            #     d = load_apnea_ref_from_annotations(setup_of_subject, db)
            #     print(d)
            #     continue
            ...
            # ax[0].set_title(
            #     str(setup_of_subject) + ' :: ' + str(corrects) + '/' + str(minlen) + ' :: ' + str(acc) + '% :: ' + str(
            #         mae) + ' :: ' + args.loss + '\nGT M(AHI): ' + str(m_AHI_gt) + ' :: Pred M(AHI): ' + str(
            #         m_AHI_pred) + ' :: M(Error): ' + str(err_AHI))
